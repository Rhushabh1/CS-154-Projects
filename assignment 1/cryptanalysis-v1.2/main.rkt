...
(define (crack-cipher strategies key) ;uses list of strategies
  (cond [(null? strategies) '()];using blank key as a start for dict-closure -- (checker '(()) key) replaces '() or (key)
        [else (let* ([subs ((car strategies) key)];list of substitutions
                     [checked-key-list (checker subs key)])
                (if (equal? '() checked-key-list) key;(crack-cipher (cdr strategies) key)
                    (append checked-key-list (crack-cipher (cdr strategies) key))))]))

(define key (build-list 26 (lambda (_) #\_)))

;;checks each substitutions
(define (checker subs k)
  (cond [(null? subs) '()]
        [else (let* ([mod-key (utils:add-substitution (car subs) k)]
                     [dict-c-mod-key (algo:dictionary-closure mod-key)])
                (cond [(equal? dict-c-mod-key #f) (checker (cdr subs) key)]
                      [else (let ([swe (algo:secret-word-enumeration dict-c-mod-key)])
                              (cond [(equal? swe #f) (checker (cdr subs) key)]
                                    [else ;(begin (utils:show-key swe)
                                     (cons swe (checker (cdr subs) key))]))]))]));swe]))]))])); --- can return partial or complete key
...
(define (crack-hard-cipher strategies key)
  '())
